...
self.onmessage = (e) => {
    ...
    // 1. Generate Height Map and get crater data
    const { heightMap, craters } = generateHeightMap(params, noise3D, seededRandom);

    // 2. Generate Planet Texture using heightmap and crater data
    const colormapData = generatePlanetTexture(params, heightMap, craters, noise2D);
...
function mapToSphere(x, y, width, height) {
    const lonRad = (x / width) * 2 * Math.PI; // 经度从 0 到 2π
    const latRad = (y / height) * Math.PI - Math.PI / 2; // 纬度从 -π/2 到 π/2
...
function generatePlanetTexture(params, heightMap, craters, noise2D) {
    // 解构出所有需要的参数
    const { 
        width, height, 
...
function calculateGradient(data, width, height) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            const y_prev = (y === 0) ? height - 1 : y - 1; // Incorrect for poles, but ok for texture
